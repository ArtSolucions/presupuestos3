...
const parseEuro = v => {
  ...
  const s=String(v).trim().replace(/\s*€\s*$/,'').replace(/\./g,'').replace(',','.');
...
function doExcel(productos,projectSecs,info){
  ...
  productos.forEach((p,idx)=>{
    ...
    [['materiales','Materiales','mat'],['fabricacion','Fabricación','tip'],['diseno','Diseño y Gestión','tip']].forEach(([key,lbl,tipo])=>{
      ...
      if(tipo==='mat'){h.push(['Proveedor','Ref','Descripción','Cant.','PVP Prov.','Dto%','Coste U.','Margen%','PVP U.','Coste Total','PVP Total']);rows.forEach(r=>h.push([r.proveedor,r.ref,r.descripcion,r.cantidad,r.pvpProv,r.dto,r.costeUnit,r.margen,r.pvpUnit,r.costeTotal,r.pvpTotal]));}
      else{h.push(['Código','Descripción','Cat.','Ud.','Horas','Min.','Cant.','Coste U.','PVP U.','Margen%','Coste Total','PVP Total']);rows.forEach(r=>h.push([r.code,r.descripcion,r.cat,r.unit,r.usaHM?r.horas:'',r.usaHM?r.minutos:'',r.cantDecimal||r.cantidad,r.costeUnit,r.pvpUnit,r.margenPct?.toFixed(1),r.costeTotal,r.pvpTotal]));}
  ...
  PROJECT_SECS.forEach(s=>{
    ...
    rows.forEach(r=>h.push([r.code,r.descripcion,r.cat,r.unit,r.usaHM?r.horas:'',r.usaHM?r.minutos:'',r.cantDecimal||r.cantidad,r.costeUnit,r.pvpUnit,r.margenPct?.toFixed(1),r.costeTotal,r.pvpTotal]));
    const c=rows.reduce((a,r)=>a+(r.costeTotal||0),0),pv=rows.reduce((a,r)=>a+(r.pvpTotal||0),0);
